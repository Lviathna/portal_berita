        ...
        <div class="flex items-center justify-between w-full lg:w-auto">
          ...
          <a href="{{ route('landing') }}">
            <div class="flex items-center gap-2">
              <img src="{{ asset('assets/img/Logo_PEI_png.png') }}" alt="Logo" class="w-8 lg:w-10">
              <p class="text-lg lg:text-xl font-bold">Portal Berita PEI</p>
            </div>
          </a>
            ...
            @foreach(\App\Models\NewsCategory::all() as $category)
                  <li>
                    <a href="{{ route('news.category', $category->slug) }}"
                      @class([
                          'hover:text-primary',
                          'text-primary' => request()->is($category->slug),
                      ])>
                      {{ $category->title }}
                    </a>
                  </li>
